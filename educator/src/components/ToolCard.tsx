...
export const ToolCard: React.FC<ToolCardProps> = ({ tool, onRatingUpdate, userVotes }) => {
  ...
  const getPriceConfig = (price: Tool['price']) => {
    switch (price) {
      case 'free':
        return { text: 'Ücretsiz', className: 'bg-green-500 text-white' };
      case 'paid':
        return { text: 'Ücretli', className: 'bg-orange-500 text-white' };
      case 'limited':
        return { text: 'Sınırlı Ücretsiz', className: 'bg-blue-500 text-white' };
      default:
        return { text: 'Ücretsiz', className: 'bg-green-500 text-white' };
    }
  };
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-xl cursor-pointer group">
      <div className="flex items-center gap-4 mb-4">
        <img 
          src={tool.logo} 
          alt={tool.name}
          className="w-16 h-16 rounded-xl object-cover border-2 border-gray-100 group-hover:border-blue-300 transition-colors"
        />
        <div>
              ...
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  <Star 
                    ...
                    className={`${
                      hasVoted 
                        ? (star <= userRating ? 'text-blue-500 fill-current' : 'text-gray-300')
                        : (star <= displayRating ? 'text-yellow-400 fill-current' : 'text-gray-300')
                    } ${hoveredStar && star <= hoveredStar && !hasVoted ? 'text-yellow-500' : ''}`} 
                  />
                </button>
              ))}
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="ml-1 font-medium text-gray-700">
                {tool.rating > 0 ? tool.rating.toFixed(1) : '0.0'} / 5
              </span>
            </div>
          </div>
          
          {hasVoted && (
            <span className="text-green-600 font-medium">✓ Oylandı</span>
          )}
        </div>
      </div>
    </div>
